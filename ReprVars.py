...
class MIP:
    ...
    def __init__(self, f, Aeq, beq, cont, filenames = False, delimiter = ','):
        """
        Can enter f, Aeq, beq, cont either as matrices of appropriate size (filenames = 0)
        Or as names to csv files, from which the data can be fetched. (filenames = 1)
        """
        self.LPSolved = False
        self.GraphDrawn = False
        self.TableGraphDrawn = False
        if filenames:
